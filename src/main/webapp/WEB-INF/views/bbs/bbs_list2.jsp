 ...
 <div id="bList_wrap">
  <table id="bList_t">
   ...
   <c:if test="${!empty blist}">
    ...
    <tr>
    <c:forEach var="b" items="${blist}" begin="4" end="7">
     <td>
      <a href="bbs_cont?bbs_no=${b.bbs_no}&page=${page}&state=cont"><img src="./upload2${b.bbs_file}" width="280" height="206" alt="."/></a>
      <%-- 웹브라우저 주소창에 노출되는 get방식으로 bbs_no,page,state 각 네임피라미터 이름에 번호,페이지 번호,구분값 cont를 담아서 전달 --%>
     </td>
  ...
  <div id="bList_paging">
   ...
   <c:if test="${(empty find_field) && (empty find_name)}">
    ...
    <c:if test="${page >1}">
     <a href="bbs_list2?page=${page-1}">&#171;</a>&nbsp;
    </c:if>
   ...
   <%-- 검색 후 페이징  --%>
    ...
    <c:if test="${page >1}">
     <a href="bbs_list2?page=${page-1}&find_field=${find_field}&find_name=${find_name}">&#171;</a>&nbsp;
    </c:if>
    ...
    <c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
     ...
     <c:if test="${a != page}">
      <a href="bbs_list2?page=${a}&find_field=${find_field}&find_name=${find_name}">${a}</a>&nbsp;
     </c:if>
    </c:forEach>
