				...
				<div class="sort-element">
					...
					<ul class="sort-menu" style="list-style-type: none;">
						...
						<li class="active"><a href="<c:url value='/bbs2_list'/>"
							title="목록 항목 선택됨">인기</a> <span style="color: gray;">
								&nbsp;| &nbsp;</span></li>
						<li><a href="<c:url value='/bbs_list'/>"><b>코스</b></a> &nbsp; &nbsp;</li>
			...
			<c:if test="${blist != null }">
				<table id="member_list" >
					...
					<c:forEach items="${blist }" var="l">
						<tr>
							<td>${l.bbs_name}</td>
							<td> <a href="bbs_cont?bbs_no=${l.bbs_no}&page=${page}&state=cont">${l.bbs_title }</a></td>
							<td>${l.bbs_hit }</td>
							<td>${l.bbs_date}</td>
							<td><input type="button" class="btn btn-secondary"
								value="수정"
								onclick="location='bbs_cont?bbs_no=${l.bbs_no}&page=${page}&state=edit';" />

							</td>
							<td><input type="button" class="btn btn-danger" value="삭제"
								onclick="location='bbs_cont?bbs_no=${l.bbs_no}&page=${page}&state=del';" /></td>
						</tr>
