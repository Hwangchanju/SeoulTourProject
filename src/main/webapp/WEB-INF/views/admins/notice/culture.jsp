...
<body
	style="width:100%; margin-left: auto; margin-right: auto; height: 1210px; max-width:1200px;">
	<hr>
	<br>
	<br>
  ...
  <path
			...
			<ul class="nav nav-pills flex-column mb-auto">
				...
				<li class="nav-item"><a
					href="<c:url value='/admin/exhibition/exhibition/1'/>"
  ...
  <path
				...
				<li class="nav-item"><a
					href="<c:url value='/admin/exhibition/culture/1'/>"
  ...
  <path
	...
	<div style="width: 1250px; margin: 50px auto 50px auto;">
		<c:forEach begin="0" step="1" end="7" var="i">
			<div class="card"
				style="width: 200px; float: left; margin-left: 10px; margin-bottom: 70px; height: 300px;">
				<img src="${list_img[i]}"
					class="card-img-top" alt="배경이미지" width="100%" height="70%"/>
				<div class="card-body">
					<a href="https://korean.visitseoul.net/${list_url[i]}"
						class="card-text">
						<h6 class="card-title">${list_title[i]}</h6>
						<small class="card-text" style="text-align: center; font-size:13px;">${list_date[i]}</small>
					</a>
				</div>
			</div>
		</c:forEach>
	...
	<div class="btn-group me-2 center" role="group"
		...
		<c:forEach begin="1" end="${paging_cnt }" var="p">
			<button type="button" class="btn btn-primary"
				onclick="location.href='<c:url value='/admin/exhibition/culture/${p }'/>';">${p }</button>
		</c:forEach>
	</div>
